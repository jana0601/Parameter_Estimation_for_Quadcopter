...
def controller(quad, extendkalmanflter,sensor,I,mass,endpoints,yaw,dt):

    observe = sensor.update(quad,dt)
    f = observe[0:3]
    w = observe[3:6]
    # get quadcopter estimated state
    state_est = extendkalmanflter.update(f,w,sensor.Y_obs2(),dt)
    ...
    Y_C = np.array(get_yc(yaw[0]))  # transform to np.array 'cause comes as np.matrix
